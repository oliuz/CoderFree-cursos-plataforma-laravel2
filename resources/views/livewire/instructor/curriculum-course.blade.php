<div>
    ...
    <x-container class="py-8 grid grid-cols-5 gap-6">
        ...
        <section class="col-span-4 text-gray-700">
            <div class="bg-white rounded-lg shadow px-10 py-6">
                    ...
                    @foreach ($sections as $section)
                        ...
                        <li class="bg-gray-100 rounded-lg shadow-lg" wire:key="section-{{ $section->id }}"
                            ...
                            <div class="px-6 py-4 cursor-move flex justify-between items-center handle">
                                ...
                                <div>
                                    <x-jet-secondary-button wire:click="editSection({{ $section->id }})"
                                        wire:target="editSection({{ $section->id }})" wire:loading.attr="disabled">
                            ...
                            <div class="px-6 pb-6">
                                ...
                                @livewire('instructor.lessons-course', ['section' => $section], key('lessons-course-' .$section->id))
                ...
                {{-- Nueva sección --}}
                <div x-data="{open: @entangle('formCreateSection.open').defer}" class="mt-6">
                    ...
                    <form wire:submit.prevent="storeSection" class="bg-gray-100 rounded-lg shadow-lg p-6 mt-4 hidden"
                        :class="{'hidden': !open}">
                        <x-jet-label class="">
                            Nueva sección
                        </x-jet-label>

                        <x-jet-input type="text" wire:model.defer="formCreateSection.name" class="w-full"
                            placeholder="Ingrese el nombre de una sección" />

                        <x-jet-input-error for="formCreateSection.name" />
    ...
    <x-jet-dialog-modal maxWidth="md" wire:model="formEditSection.open">
        <x-slot name="title">
            Editar seccion
        </x-slot>
        ...
        <x-slot name="content">
            ...
            <x-jet-input type="text" wire:model.defer="formEditSection.name" class="w-full" />
            <x-jet-input-error for="formEditSection.name" />
        </x-slot>

        <x-slot name="footer">
            <x-jet-danger-button wire:click="cancelEditSection" wire:loading.attr="disabled"
                wire:target="cancelEditSection">
                Cancelar
            </x-jet-danger-button>

            <x-jet-button wire:click="updateSection" wire:loading.attr="disabled" wire:target="updateSection">
                Actualizar
            </x-jet-button>
        </x-slot>
    </x-jet-dialog-modal>
            ...
            Livewire.on('delete-section', sectionId => {
                ...
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        ...
                        Livewire.emitTo('instructor.curriculum-course', 'deleteSection', sectionId);
